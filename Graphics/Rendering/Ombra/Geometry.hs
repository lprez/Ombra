...
mkGeometry3D' :: (GLES, Attributes (Append (i ': is) Geometry3D))
              => [Triangle (Vertex (i ': is), Vec3, Vec2, Vec3)]
              -> Geometry (Append (i ': is) Geometry3D)
...
mkGeometry2D' :: (GLES, Attributes (Append (i ': is) Geometry2D))
              => [Triangle (Vertex (i ': is), Vec2, Vec2)]
              -> Geometry (Append (i ': is) Geometry2D)
...
vertex2D :: (GLES, Monad m)
         => Vec2
         -> Vec2
         -> GeometryBuilderT Geometry2D m (AttrVertex Geometry2D)
...
vertex3D :: (GLES, Monad m)
         ...
         -> Vec2
         -> Vec3
         -> GeometryBuilderT Geometry3D m (AttrVertex Geometry3D)
vertex3D p u n = vertex $ mkVertex3D p u n

mkVertex3D :: GLES => Vec3 -> Vec2 -> Vec3 -> Vertex Geometry3D
mkVertex3D p u n = Attr D3.Position3 p :~ Attr D3.UV u :~ Attr D3.Normal3 n

mkVertex2D :: GLES => Vec2 -> Vec2 -> Vertex Geometry2D
